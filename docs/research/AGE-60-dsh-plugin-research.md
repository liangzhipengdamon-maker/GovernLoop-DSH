...
| `agent/session-start` | emit | `{agent, source}` (`startup`/`resume`) | once, before first turn; "first supported startup injection point" | inject review/context into a resumed session |
...
Notes: `tools/pre-execute` **cannot rewrite `exec.arguments`** (logged args would desync). `ask` is serviced by `ctx.approval` when mounted, **fails closed to `deny`/`unavailable` otherwise**. `tool/call` records the model's raw `arguments` string verbatim — policy reads the same string the model produced. There is no identifier named `tool/before-execute` or `tool/after-execute` anywhere in the shipped code — the vocabulary is exactly the four events above plus `tools/code-dispatch-log` and `tools/change`.
...
2. **Can BEFORE_DESTRUCTIVE_ACTION be enforced at runtime?** **Yes, at the tool boundary.** `tools/pre-execute` returns `deny`/`ask` and is executed by the registry before dispatch; `ctx.tools.guard()` adds monotonic owner policy that later listeners cannot overturn; `ask` routes to `ctx.approval`. The sandbox (`sandbox-policy`, per-session `sandbox/mode`) enforces file effects independently of the model. This is real enforcement, not prompting — BUT it cannot inspect `exec.arguments`-rewritten input (no rewrite allowed) and only sees the model's raw arguments string; classification quality depends on the plugin's rules. It also cannot block *conceptual* destructive actions that don't go through DSH tools (e.g. actions performed by an external process the agent launched) — sandbox confinement is the backstop for those.
